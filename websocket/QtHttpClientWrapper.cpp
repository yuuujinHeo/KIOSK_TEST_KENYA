...
//                            qDebug () << "HTTP header :" << header
//                                      << "value :"  << value;
                            m_currentRequest->addHeader (header, value);
                            if(header.toUpper().contains((QtHttpHeader::ContentLength).toUpper()))
                            {
                                bool ok  = false;
...
//                        qDebug () << "HTTP end of headers";
                ...
                case AwaitingContent: { // raw data × N (until EOF ??)
                    QByteArray raw = line.trimmed ();
//                    qDebug() << QString("http Receive Data:%1") .arg(QString::fromUtf8(raw.data()));
                    m_currentRequest->appendRawData (line);
//                    qDebug () << "HTTP size :"    << m_currentRequest->getRawData().size();
                    if (m_currentRequest->getRawDataSize () == m_currentRequest->getHeader(QtHttpHeader::ContentLength).toInt())
                    {
//                        qDebug () << "HTTP end of content";
                        m_parsingStatus = RequestParsed;
                    }
                    break;
                }
...
QString QtHttpClientWrapper::getPeerIP(){
    return m_sockClient->peerAddress().toString();
}
